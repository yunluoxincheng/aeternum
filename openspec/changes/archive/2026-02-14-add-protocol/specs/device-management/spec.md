...
**Capability ID**: `device-management`
**Version**: 1.0
**Status**: Proposed
...
- 依赖: `models::device::DeviceHeader` - DeviceHeader 类型
- 依赖: `models::device::DeviceStatus` - DeviceStatus 枚举
- 依赖: `crypto::kem::KyberKEM` - KEM 封装/解封
- 被依赖: `recovery-protocol` - 撤销操作可能触发恢复流程
- 被依赖: `epoch-upgrade` - Epoch 升级需要更新所有设备的 Header
